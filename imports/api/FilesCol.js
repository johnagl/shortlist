...
const UserFiles = new FilesCollection({
  ...
  onBeforeUpload(file) {
    if (file.size <= 10485760 && /png|jpg|pdf|docx|doc|jpeg/i.test(file.extension)) return true;
    return alert('Please upload an image, pdf, or word document, that has size equal to or less than 10MB.');
  },
  onAfterUpload(image) {
    // Move file to GridFS
